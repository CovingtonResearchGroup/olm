...
def readPhreeqcOutput(phreeqcOutputFile):
    ...
    Notes
    -----
    While this function can be used on its own. It is primarily designed to be used by
    the processMidf function, which provides a more convenient interface.
    """
            ...
            if not (endOfRun):
                ...
                if block == "beginning":
                    ...
                    if "Description of solution" in line:
                        ...
                        simulationDict[linesplit[0]] = to_numeric(
                            linesplit[1], errors="coerce"
                        )
                    ...
                    if "Electrical balance" in line:
                        ...
                        linesplit = linestrip.split()
                        balance = linesplit[4]
                        simulationDict["Electrical balance"] = to_numeric(
                            balance, errors="coerce"
                        )
                    if "Percent error" in line:
                        linestrip = line.strip()
                        linesplit = linestrip.split()
                        error = linesplit[4]
                        simulationDict["Percent error"] = to_numeric(
                            error, errors="coerce"
                        )
                    if "Distribution of species" in line:
                        block = "distribution"
                elif block == "distribution":
                    if not ("Species" in line) and not ("Log" in line):
                        linestrip = line.strip()
                        linesplit = linestrip.split()
                        if "Saturation indices" in line:
                            ...
                            simulationDict[linesplit[0] + "_Molality"] = to_numeric(
                                linesplit[1], errors="coerce"
                            )
                            simulationDict[linesplit[0] + "_Activity"] = to_numeric(
                                linesplit[2], errors="coerce"
                            )
                elif block == "SI":
                    if not ("Phase" in line):
                        ...
                        linesplit = linestrip.split()
                        if len(linesplit) == 5:
                            simulationDict["SI_" + linesplit[0]] = to_numeric(
                                linesplit[1], errors="coerce"
                            )
...
def writePhreeqcInput(
    ...
    datetext : str
       String containing the text that describes the date as it should be written
       into the PHREEQC input file.

    charge : str
        String containing name of element (or pH) that should be adjusted to obtain
        charge balance. This is done internally by PHREEQC. (Default=None)
    Returns
    -------
    status : int
    ...
    try:
        ...
        for characteristic in phreeqcDict.keys():
            # Check to see if this characteristic is in our DataFrame
            if characteristic in list(sample_row.keys()):
                ...
                if not isnan(sample_row[characteristic]):
                    if phreeqcDict[characteristic] == charge:
                        print(
                            phreeqcDict[characteristic]
                            + " "
                            + str(sample_row[characteristic])
                            + " "
                            + "charge",
                            file=phreeqc_input_file,
                        )
                    else:
                        print(
                            phreeqcDict[characteristic]
...
def phreeqcRunSetPCO2(
    ...
    newInputFile : string
        The name of the set-PCO2 file to create and run. If not specified, the final 5
        characters will be stripped off of phreeqcInputFile and 'set-PCO2.phrq' will be
        added.
    Returns
    -------
    simulationDict : Dict
        A dictionary that contains the output of readPhreeqcOutput() run on the set PCO2
        simulation.
    """
    ...
    for linenum, input_line in enumerate(input_file_text):
        ...
        if "pH" in input_line:
            if not wrote_CO2_line:
                ...
                print(
                    "There is more than one line with pH. This code only works for PHREEQC inputs with a single solution. Check input file!"
                )
        ...
        for linenum, input_line in enumerate(input_file_text):
            if "SOLUTION" in input_line:
                if not wrote_CO2_line:
                    ...
                    print(
                        "There is more than one line with SOLUTION. This code only works for PHREEQC inputs with a single solution. Check input file!"
                    )
...
def calciteSaturationAtFixedPCO2(
    ...
    """
    simulationDict = phreeqcRunSetPCO2(
        logPCO2,
        phreeqcInputFile,
        PHREEQC_PATH,
        DATABASE_FILE,
    ...
    return simulationDict["SI_Calcite"]


def findPCO2atCalciteSaturation(
    phreeqcInputFile, PHREEQC_PATH, DATABASE_FILE, newInputFile=None, units="atm"
):
    """
    Finds the PCO2 where calcite would be saturated by modifing PCO2 of a PHREEQC input
    file.
    ...
    units: string
        The units in which to return the partial pressure of CO2. Currently, 'ppm' and
        'atm' are allowed (Default=atm).
    Returns
    -------
    PCO2 : float
    ...
    eqLogPCO2 = brentq(calciteSaturationAtFixedPCO2, -6.0, 0.0, args=args, xtol=0.01)
    if units == "ppm":
        PCO2 = 1e6 * 10.0 ** eqLogPCO2
    elif units == "atm":
        PCO2 = 10.0 ** eqLogPCO2
    else:
...
def processMidf(
    ...
    """
    Takes a WQXtoPandas site multi-indexed DataFrame and runs all samples through
    ...
    for date in dates:
        sample_row = df.loc[date]
        datetext = date.date().isoformat()
        if not (os.path.exists(site_dir)):
            try:
                os.makedirs(site_dir)
            except os.error:
                print(("Problem creating location directory: " + site_dir))
                return -1
        if force_balance == "":
            ...
            phreeqc_file_name = os.path.join(
                site_dir, datetext + "-" + force_balance + ".phrq"
            )
        ...
        if retcode == 0:
            ...
            # This catches cases where PHREEQC fails (e.g. doesn't converge)
            simulationDict = None
        if force_balance == "Alk" and simulationDict is not None:
            # Force charge balance on Alkalinity
            alk_converged = False
            number_of_iterations = 0
            max_iterations = 10
            while not (alk_converged):
                if simulationDict is not None:
                    # Retrieve charge balance error
                    balance_eq = simulationDict["Electrical balance"]
                    print("balance_eq=", balance_eq)
                    if "Alkalinity" in simulationDict:
                        alk = simulationDict["Alkalinity"]
                    else:
                    ...
                    New_alk_molL = (
                        alk + balance_eq
                    )  # attempt to stabilize this using a factor of 0.9. Otherwise it seems to overshoot. this didn't work.
                    if (
                        New_alk_molL < 0
                    ):  # PHREEQC won't take negative alkalinity inputs
                        New_alk_molL = alk * 0.5
                    print("new alk=", New_alk_molL)
                    New_alk_mgL = 0.5 * molL_to_mgL(
                        New_alk_molL, "CaCO3"
                    )  # PHREEQC wants it as 0.5CaCO3
                    ...
                    if retcode == 0:
                        ...
                        if abs(simulationDict["Percent error"]) < 5.0:
                            alk_converged = True
                    ...
                    if number_of_iterations > max_iterations:
                        simulationDict = (
                            None  # This will cause this case to be thrown out below
                        )
                else:
                    alk_converged = True
                    ...
                    reason = (
                        "PHREEQC encountered an error while processing this sample. Return code = "
                        + str(retcode)
                    )
        if simulationDict is not None:
    ...
    phreeqc_df = DataFrame(phreeqc_data_list, index=output_dates)
    if force_balance == "Alk":
        print(
            "Number of converged alkalinity forced charge balance cases = ",
            num_converged,
        )
...
def processSites(
    ...
    bracket_charge_balance : bool
       If set to True, then charge balance will be bracketed by forcing balance on Ca
       and Alkalinity alternately. Default = False.
    process_regular : bool
       If set to True, then PHREEQC will be run without any charge balance forcing.
       This can be set to False if the calculations without forced balance are already
       done and you only want to do the charge balance runs. Default = True.
    ...
    if process_regular:
        ...
        for site, site_midf in sitesDict.items():
    ...
    if bracket_charge_balance:
        ...
        for site, site_midf in sitesDict.items():
            ...
            phreeqc_site_file_ca = os.path.join(
                sitesDir, site, site + "-PHREEQC-Ca.pkl"
            )
            try:
            ...
            phreeqc_site_file_alk = os.path.join(
                sitesDir, site, site + "-PHREEQC-Alk.pkl"
            )
            try:
